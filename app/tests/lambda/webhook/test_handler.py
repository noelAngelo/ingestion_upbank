...
logger.add('test_handler.log', format=DEFAULT_LOG_FORMAT)


def test_handler_empty_event():
    event = None  # Mock event data if needed
...
def test_handler_empty_webhook_data():
    ...
    assert (
        json.loads(response["body"])["message"]
        == "No webhook payload found in the request body"
    )
